...
| **Mobile Broadcaster (Phone GPS + IMU + Live Camera)** | `frontend/src/app/phone/page.tsx` | `3001` HTTP POST<br>`/api/telemetry` | `frontend/src/app/phone/page.tsx` (`/phone`) | 🟢 **MOUNTED & LIVE** |
| **WorksiteGuard YOLO Camera Mesh** | `frontend/yolo/worksite-guard/worksite-guard/server/main.py` | `8000` WebSocket<br>`/ws/dashboard`<br>`/ws/client` | `frontend/src/app/reports/page.tsx`<br>`WorksiteGuardDashboard.tsx` | 🟢 **MOUNTED & LIVE (Step 3 in start.bat)** |
| **MUSt3R 3D SLAM & Spatial Mesh** (Multi-View Stereo Odometry) | `backend/app/slam_service.py`<br>`slam/must3r/get_reconstruction.py`<br>`slam/must3r/slam.py` | `8001` / `3001` REST<br>`/api/slam/scene`<br>`/api/slam/reconstruct` | `frontend/src/app/slam/page.tsx` (`/slam`)<br>`Must3rViewer.tsx` (Three.js) | 🟢 **MOUNTED & LIVE** |
