...
class SLAM_MUSt3R():
    ...
    def write_pointcloud(self, path, conf_thr=1.0, max_points=2_000_000):
        """Write keyframe pointmaps as a browser-friendly binary PLY file."""
        chunks = []
        for pointmap in self.keyframe_pointmaps:
            points, colors, confidence = pointmap
            ...
            colors = colors[mask]
            if len(points):
                chunks.append((points, colors))

        if not chunks:
            raise RuntimeError("No keyframe points meet the confidence threshold")

        points = torch.cat([chunk[0] for chunk in chunks])
        colors = torch.cat([chunk[1] for chunk in chunks])
        ...
        colors = colors.clamp(0, 1).mul(255).byte().numpy()
