...
def set_execution_mode(inputfiles, execution_mode, num_mem_images, render_once, vidseq_local_context_size, keyframe_interval, slam_local_context_size, slam_subsample, min_conf_keyframe, keyframe_overlap_thr, overlap_percentile):
    ...
    num_mem_images = gradio.Slider(label="Number of memory images", value=num_mem_images,
                                   minimum=num_mem_images, maximum=max(num_mem_images + 1, 1), step=1, visible=False)
    ...
    vidseq_local_context_size = gradio.Slider(label="Local context size", value=vidseq_local_context_size,
                              minimum=vidseq_local_context_size, maximum=max(vidseq_local_context_size + 1, 1), step=1, visible=False)
    keyframe_interval = gradio.Slider(label="Keyframe Interval", value=keyframe_interval,
                          minimum=keyframe_interval, maximum=max(keyframe_interval + 1, 1), step=1, visible=False)

    # vidslam
    slam_local_context_size = gradio.Slider(label="Local context size", value=slam_local_context_size,
                                            minimum=slam_local_context_size, maximum=max(slam_local_context_size + 1, 1), step=1, visible=False)
    ...
    if execution_mode in ["linseq", "retrieval"]:
        ...
        num_mem_images = gradio.Slider(label="Number of memory images", value=current_num_mem_images,
                                       minimum=1, maximum=max(num_files, 2), step=1, visible=True)
    ...
    with gradio.Blocks(css=""".gradio-container {margin: 0 !important; min-width: 100%};""", title="MUSt3R Demo") as demo:
        ...
        with gradio.Column():
            ...
            if embed_viser:
                ...
                viser_port = viser_server.server.get_port()
                viser_html = gradio.HTML(f"""<div style="width:100%; height:600px; border:1px solid #e4e4e7; border-radius: 4px; resize:vertical; overflow:auto;">
                    <div style="padding: 5px 12px; display:flex; justify-content:space-between; align-items:center; background:#111827;">
                        <span style="color: #94a3b8; font-weight:700; font-size:12px;">MUSt3R 3D Visualization</span>
                        <span><a id="viser-fs-link" href="http://{viser_server.address}/?fixedDpr=1" target="_blank" style="color:#38bdf8; font-size:12px; text-decoration:none; font-weight:700;">Full screen ↗</a></span>
                    </div>
                    <iframe
                        id="viser-embed-frame"
                        src="http://{viser_server.address}/?fixedDpr=1"
                        style="width:100%; height: calc(100% - 36px); border:none; background:#0a0f1d;">
                    </iframe>
                    <script>
                        (function() {{
                            try {{
                                ...
                                var port = '{viser_port}';
                                var target = 'http://' + host + ':' + port + '/?fixedDpr=1';
                                var frame = document.getElementById('viser-embed-frame');
                                if (frame && frame.src.indexOf('0.0.0.0') !== -1) frame.src = target;
                                var link = document.getElementById('viser-fs-link');
                                if (link && link.href.indexOf('0.0.0.0') !== -1) link.href = target;
                            }} catch(e) {{}}
                        }})();
                    </script>
                    </div>""")
